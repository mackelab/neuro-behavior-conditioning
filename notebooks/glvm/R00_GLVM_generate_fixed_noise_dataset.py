...
draw_gaussian = True
fixed_C_across_datasets = True
fixed_noise_across_samples = True
fixed_noise = np.linspace(0.1, 1.9, args.list_len)
c_list = []
...
if fixed_C_across_datasets:
    c_list = [c_list[0]] * args.list_len

noise_list = []
for i in range(args.list_len):
    if fixed_noise_across_samples:
        print(f"Fixed noise: {fixed_noise[i]}")
        noi = fixed_noise[i] * np.ones(args.n_data_dims)
    elif draw_gaussian:
...
for c_id, (C, noise) in enumerate(zip(c_list, noise_list)):
    ...
    print(f"Saving dataset {c_id}")
    print(C, noise)

    # save the datasets
    datapath = f"../../data/rev_glvm/{args.x_dim}_dim_{args.z_dim}_dim/"
