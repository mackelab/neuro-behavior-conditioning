...
flag_decoder = "_flexible_dec_different_ds_range"
# dataset_size = [100, 1000, 3000, 5000, 7000, 9000]
dataset_size = [1, 10, 30, 60, 100, 1000, 5000, 9000]

# runs/summary/R_all_runs_dataset_size_flexible_dec_different_ds_range.txt runs/summary/R_all_runs_dataset_size_frozen_dec_different_ds_range.txt
# load txt file as a list
with open(
    os.path.join(f"./runs/summary/R_all_runs_dataset_size{flag_decoder}.txt"), "r"
) as f:
    dataset_size_runs = f.read().splitlines()
...
for run_id, run in enumerate(dataset_size_runs):
...
runs = dataset_size_runs  # runs_all_obs + runs_masked
...
# gt variance
mean_gt_dict = {
    ds: {i: copy.deepcopy(dicts) for i in range(n_masks)} for ds in dataset_size
}
# posterior variance
var_dict = {
    ds: {
...
# %%
mean_mask_ed = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
mean_enc = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
mean_zero = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}

var_mask_ed = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
var_enc = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
var_zero = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}

mean_gt = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
var_gt = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}


mean_gt_all = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
var_gt_all = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}


condition_list = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
C_list = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
d_list = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
noise_list = {ds: {i: [] for i in range(n_masks)} for ds in dataset_size}
            ...
            if os.path.isdir(os.path.join(args.fig_root, args.dir, run_condition)):
                if (
                    ...
                    try:
                        ...
                        # dataset size
                        ds_size = argsread.train_samples
...
condition_list = {ds: {i: [] for i, _ in enumerate(methods)} for ds in dataset_size}
...
for run_id, run in enumerate(runs):
    ...
    for i, method in enumerate(methods):
        ...
        ds_size = model.args.train_samples
