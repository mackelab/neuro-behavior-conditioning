...
def main(args):
    ...
    with open("./configs/glvm/fit_config_revisions_mask_size.yml", "r") as f:
    ...
    for c_id in range(args.list_len):
        dataset_train = torch.load(
            f"{data_directory:s}/glvm/20_dim_1_dim/data_train_{c_id:d}{flag:s}.pt"
        )
        dataset_valid = torch.load(
            f"{data_directory:s}/glvm/20_dim_1_dim/data_valid_{c_id:d}{flag:s}.pt"
        )
        dataset_test = torch.load(
            f"{data_directory:s}/glvm/20_dim_1_dim/data_test_{c_id:d}{flag:s}.pt"
        )

        # ---------- check the effect of the number of masked values ----------------
        for masked_val in range(20):
            # for masked_val in [1, 10, 19]:
            # update the number of masked values
            args.n_masked_vals = masked_val
            ...
            for i, method in enumerate(methods):
                ...
                args.method = method
                name_tag = f"masked_{args.n_masked_vals}_xdims_{args.x_dim}_C_{args.C[0][0]:.2f}_sig_{args.noise[0][0]:.2f}"
                ...
                save_run_directory(
                    method, args, str(model.ts), "./runs_rev/"
